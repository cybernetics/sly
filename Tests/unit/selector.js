...
test("element", function() {
	...
	ok( Sly.search("*").length >= 30, "Select all" );
	var all = Sly.search("*"), good = true;
	...
	// equals( Sly.search("#object1 param").length, 2, "Object/param as context" );

	/* WRONG, results from multiple contexts are not sorted AND filtered, but comma separated items? Sorry, No!
	isSet( Sly.search("p", document.getElementsByTagName("div")), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context." );
	isSet( Sly.search("p", "div"), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context." );
	isSet( Sly.search("p", Sly.search("div")), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context." );
	 */
	isSet( Sly.search("p", document.getElementsByTagName("div")), q("firstp", "ap", "sndp", "en", "sap", "first"), "Finding elements with a context." );
	isSet( Sly.search("p", "div"), q("firstp", "ap", "sndp", "en", "sap", "first"), "Finding elements with a context." );
	isSet( Sly.search("p", Sly.search("div")), q("firstp", "ap", "sndp", "en", "sap", "first"), "Finding elements with a context." );

	ok( Sly.search("#length").length, '&lt;input name="length"&gt; cannot be found under IE, see #945' );
	ok( Sly.search("#lengthtest input").length, '&lt;input name="length"&gt; cannot be found under IE, see #945' );

	// Check for unique-ness and sort order
	isSet( Sly.search("*"), Sly.search("*, *"), "Check for duplicates: *, *" );
	isSet( Sly.search("p"), Sly.search("p, div p"), "Check for duplicates: p, div p" );
...
if (false && location.protocol != "file:" ) {
	test("XML Document Selectors", function() {
		...
		jQuery.get("data/with_fries.xml", function(xml) {
			equals( Sly.search("foo_bar", xml).length, 1, "Element Selector with underscore" );
			equals( Sly.search(".component", xml).length, 1, "Class selector" );
			equals( Sly.search("[class*=component]", xml).length, 1, "Attribute selector for class" );
			equals( Sly.search("property[name=prop2]", xml).length, 1, "Attribute selector with name" );
			equals( Sly.search("[name=prop2]", xml).length, 1, "Attribute selector with name" );
			equals( Sly.search("#seite1", xml).length, 1, "Attribute selector with ID" );
			equals( Sly.search("component#seite1", xml).length, 1, "Attribute selector with ID" );
			start();
		});
	});
}
...
test("id", function() {
	...
	equals( Sly.search("#tName1")[0].id, 'tName1', "ID selector with same value for a name attribute" );
	equals( Sly.search("#tName2").length, 0, "ID selector non-existing but name attribute on an A tag" );
	t( "ID Selector on Form with an input that has a name of 'id'", "#lengthtest", ["lengthtest"] );

	t( "ID selector with non-existant ancestor", "#asdfasdf #foobar", [] ); // bug #986

	isSet( Sly.search("div#form", document.body), [], "ID selector within the context of another element" );
...
test("class", function() {
	...
	isSet( Sly.search(".blog", document.getElementsByTagName("p")), q("mark", "simon"), "Finding elements with a context." );
	isSet( Sly.search(".blog", "p"), q("mark", "simon"), "Finding elements with a context." );
	isSet( Sly.search(".blog", Sly.search("p")), q("mark", "simon"), "Finding elements with a context." );
	// isSet( Sly.search(".blog", "p").find(), q("mark", "simon"), "Finding elements with a context." );
	...
	isSet( Sly.search(".e", div), [ div.firstChild ], "Finding a second class." );

	div.lastChild.className = "e";

	isSet( Sly.search(".e", div), [ div.firstChild, div.lastChild ], "Finding a modified class." );
});
...
test("name", function() {
	...
	isSet( Sly.search("#form input[name=action]"), q("text1"), "Name selector within the context of another element" );
	isSet( Sly.search("#form input[name='foo[bar]']"), q("hidden2"), "Name selector for grouped form element within the context of another element" );
...
test("child and adjacent", function() {
	...
	isSet( Sly.search("> :first", document.getElementById("nothiddendiv")), q("nothiddendivchild"), "Verify child context positional selctor" );
	isSet( Sly.search("> :eq(0)", document.getElementById("nothiddendiv")), q("nothiddendivchild"), "Verify child context positional selctor" );
	isSet( Sly.search("> *:first", document.getElementById("nothiddendiv")), q("nothiddendivchild"), "Verify child context positional selctor" );
...
test("attributes", function() {
	...
	Sly.search("form input")[0].test = 0;
	Sly.search("form input")[1].test = 1;
...
test("pseudo (:) selectors", function() {
	...
	var $div = jQuery(Sly.search('#nothiddendivchild'));
